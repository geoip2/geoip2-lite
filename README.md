...
[![Build Status](https://app.travis-ci.com/geoip2/geoip2-lite.svg?branch=main "GeoIP2 on Travis")](https://app.travis-ci.com/geoip2/geoip2-lite) [![Test](https://github.com/geoip2/geoip2-lite/workflows/Test/badge.svg "GeoIP2 on GitHub")](https://github.com/geoip2/geoip2-lite) [![Latest Version](https://img.shields.io/npm/v/geoip2-lite.svg "Latest Version")](https://www.npmjs.com/package/geoip2-lite) [![Downloads](https://img.shields.io/npm/dm/geoip2-lite.svg "Downloads")](https://www.npmjs.com/package/geoip2-lite)
...
GeoIP2-lite instead attempts to be a fully native JavaScript library.  A converter script converts the CSV files from MaxMind into
an internal binary format (note that this is different from the binary data format provided by MaxMind).  The geoip2 module uses this
...
### 1. get the library
    ...
    $ npm install geoip2-lite

### 2. update the datafiles (optional)

Run `cd node_modules/geoip2-lite && npm run-script updatedb license_key=YOUR_LICENSE_KEY` to update the data files. (Replace `YOUR_LICENSE_KEY` with your license key obtained from [maxmind.com](https://support.maxmind.com/hc/en-us/articles/4407111582235-Generate-a-License-Key))
...
There are no plans to change this.  `geoip2-lite` stores all data in RAM in order to be fast.

API
---

geoip2-lite is completely synchronous.  There are no callbacks involved.  All blocking file IO is done at startup time, so all runtime
...
considered internal to `geoip2-lite`.  To get a human readable format, pass them to `geoip.pretty()`
...
This method returns a string if the input was in a format that `geoip2-lite` can recognise, else it returns the
input itself.
...
```bash
node ./node_modules/geoip2-lite/scripts/updatedb.js license_key=YOUR_LICENSE_KEY
```

### Ways to reload data in your app when update finished ###

If you have a server running `geoip2-lite`, and you want to reload its geo data, after you finished update, without a restart.
...
This tool can be used with `npm run-script node-modules/geoip2-lite/updatedb` to periodically update geo data on a running server.
...
geoip2-lite understands.  You will need to use the `.csv` files from MaxMind for conversion.
...
`geoip2-lite` is Copyright 2022 Chandra Gupta <i@chandragupta.me> and the latest version of the code is
available at https://github.com/geoip2/geoip2-lite

License
-------

There are two licenses for the code and data.  See the [LICENSE](https://github.com/geoip2/geoip2-lite/blob/main/LICENSE) file for details.
